...
import { SortableContainerContext, SortableContainer } from "../../core/sortable-container"
...
export const StatusLaneContainer = <T extends StatusLaneItem>({items, setItems, columns, columnsToIncludeInProgressBar, children}: StatusLaneContainerProps<T>): JSX.Element => {
	const [graph, setGraph] = useState<GraphComponent>(() => ProgressBarMultiValue);
	const Graph = graph;
	
	const getItemAmount = (_items: T[], columnIdFilter?: UniqueIdentifier): number => {
		const filtered = columnIdFilter !== undefined ? _items.filter(item => item.columnId === columnIdFilter) : _items;
		...
		<SortableContainerContext items={items} setItems={setItems}>
			...
			<div className="mt-5 flex flex-col gap-4">
				...
				<div className="mt-5 flex gap-4">
					{columns.map((column, i) => <StatusLane key={i} {...column} items={sortItems.filter(item => item.columnId === column.id)}>
							{children}
						</StatusLane>)}
						<DragOverlay>
...
export const StatusLane = <T extends SortableContainerItem>({id, label, fill, items, children}: StatusLaneProps<T>): JSX.Element => {
