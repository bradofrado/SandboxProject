...
import { type NotifyLabelProps, NotifyLabel } from './notify-label';
...
export const SidePanel: React.FunctionComponent<SidePanelProps> = ({className, items, children, onBodyClick, path}) => {
    const queryToString = (query: ParsedUrlQueryInput): string => {
        return Object.entries(query).map(values => values.join('=')).join('&');
    }
    return <div className="flex flex-1">
            <div className={`${className || ''} z-40 max-w-64 transition-transform sm:translate-x-0`}>
                            ...
                            {items.map((item, i) => {
                                ...
                                return <li key={i}>
                                        <a aria-selected={selected} className="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-primary-light dark:hover:bg-primary-light group aria-selected:bg-primary-light dark:aria-selected:bg-primary-light" href={link}>
                                            {Icon ? <Icon className="w-5 h-5 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-primary dark:group-hover:text-white group-aria-selected:text-primary dark:group-aria-selected:text-white"/> : null}
                                            <span className="ml-3 flex-1">{item.label}</span>
                                            {item.notifyLabelItem ? <NotifyLabel {...item.notifyLabelItem}/> : null}
                                        </a>
                                    </li>
                            })}
                        </ul>
