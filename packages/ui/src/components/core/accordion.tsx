...
export const Accordion: React.FunctionComponent<AccordionProps> = ({items}) => {
    const [selected, setSelected] = useState<number>(-1);
		const onSelect = (value: boolean, i: number): void => {setSelected(value ? i : -1)}
    return (
        <div className={`group ${selected}`}>
            {items.map((item, i) => <Fragment key={i}>
							<ExpandableComponent label={LabelContent(item)} onChange={(value) => {onSelect(value, i)}} value={selected === i}>
								{item.content}
							</ExpandableComponent>
            </Fragment>)}
        </div>
...
export const ExpandableComponent: React.FunctionComponent<ExpandableComponentProps> = ({value, onChange, label, children}) => {
	return (
	<div className="flex flex-col gap-2">
		<button onClick={() => {onChange && onChange(!value)}} type="button">
			{label}
		</button>
...
const LabelContent = (item: AccordionItem): React.ReactNode => 'labelContent' in item ? item.labelContent : <>
