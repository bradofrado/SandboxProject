...
export const ReportingView: React.FunctionComponent = () => {
	...
	return (
		<div className="p-8">
			...
			<StatusLaneContainer columns={columns} columnsToIncludeInProgressBar={columnsToIncludeInProgressBar} items={items} setItems={setItems} >
...
const ARClientCard: React.FunctionComponent<ARClientCardProps> = ({name, description, amount, status, profileImage, outline}) => {
	if (outline) {
	...
	return (
		<div >
			<Card items={[{name: 'Edit', id: '0'}]} label={labelPill} onChange={() => undefined}>
