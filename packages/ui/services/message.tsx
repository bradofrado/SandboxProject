...
export type MessageContextType = [Message[], (message: string, user: User) => void]
...
export const MessageProvider = ({children}: MessageProviderProps) => {
    ...
    const sendMessage = (message: string, user: User) => {
        ...
        copy.push({id: '3', userId: user.id, name: user.name, text: message, avatar: user.image});
        setMessages(copy);
    }
    return <MessageContext.Provider value={[_messages, sendMessage]}>
        {children(_messages)}
